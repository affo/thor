...
from thor.generated.ThorVisitor import ThorVisitor
from thor.exceptions import VisitorException
...
class Visitor(ThorVisitor):
    ...
    def visitProgram(self, ctx:ThorParser.ProgramContext):
    ...
    def visitUnaryMinus(self, ctx:ThorParser.UnaryMinusContext):
    ...
    def visitMathHigh(self, ctx:ThorParser.MathHighContext):
        return self.visitBinaryMath(ctx)

    def visitMathLow(self, ctx:ThorParser.MathLowContext):
        return self.visitBinaryMath(ctx)

    def visitNumberLiteral(self, ctx:ThorParser.NumberLiteralContext):
        e = Expression(ctx.NUMBER().getText(), Expression.Type.NUMBER)
        if self.debug:
            self.debug_info.append(e)
        return e

    def visitStringLiteral(self, ctx:ThorParser.StringLiteralContext):
        e = Expression(ctx.STRING().getText(), Expression.Type.STRING)
        if self.debug:
            self.debug_info.append(e)
        return e

    def visitNested(self, ctx:ThorParser.NestedContext):
        e = self.visit(ctx.expression())
        if self.debug:
            self.debug_info.append(e)
        return e

    def visitAssignment(self, ctx:ThorParser.AssignmentContext):
        e = self.visit(ctx.expression())
    ...
    def visitIdentifier(self, ctx:ThorParser.IdentifierContext):
        ident = ctx.ID().getText()
    ...
    def visitPrintStmt(self, ctx:ThorParser.PrintStmtContext):
        e = self.visit(ctx.expression())
        print(e.value)

    def visitFnLiteral(self, ctx:ThorParser.FnLiteralContext):
        f = Function(
            ctx,
            self.scope.copy(),
    ...
    def visitFnCall(self, ctx:ThorParser.FnCallContext):
    ...
    def visitReturnStmt(self, ctx:ThorParser.ReturnStmtContext):
        if not self.is_fn:
    ...
    def visitFnGetStream(self, ctx:ThorParser.FnGetStreamContext):
        l = self.visit(ctx.left)
    ...
    def visitStreamRead(self, ctx:ThorParser.StreamReadContext):
    ...
    def visitStreamWrite(self, ctx:ThorParser.StreamWriteContext):
    ...
    def visitCloseStmt(self, ctx:ThorParser.CloseStmtContext):
